...
object UdfRegister {
  ...
  def register(udfName: String, udfClassFullname: String, udfJarPath: String, session: SparkSession, ifLoadJar: Boolean) {
    ...
    synchronized {
      ...
      val functionIdentifier: FunctionIdentifier = new FunctionIdentifier(udfName)
      if (session.sessionState.functionRegistry.functionExists(functionIdentifier)) session.sessionState.functionRegistry.dropFunction(functionIdentifier)

      registerUdf(paramCount, session, udfName,functionIdentifier, udfClassFullname, returnDataType)
    }
  }
  ...
  private def registerUdf(paramCount: Int, session: SparkSession, udfName: String,functionIdentifier: FunctionIdentifier, udfClassName: String, returnDataType: DataType): Unit = {
    ...
    paramCount match {
      case 0 =>
        ...
        session.sessionState.functionRegistry.registerFunction(
          functionIdentifier,
          (e: Seq[Expression]) => new ScalaUDF(f, returnDataType, e,Nil,None))
      case 1 =>
