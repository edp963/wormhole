...
case class WhHdfsLogPath(root: String, ns: UmsNamespace, folder: WhHdfsLogPathFolder) {
  private val base = WhHdfsLogPathBase.STG.toString
  private val nsStr = UmsNamespace.namespacePath(ns)

//  lazy val nsPath = List(
//    FileUtils.pf(root),
//    FileUtils.pf(base),
//    FileUtils.pf(nsStr)).mkString("/").toLowerCase

//  lazy val fullPath = List(
//    FileUtils.pf(nsPath),
//    FileUtils.pf(List(DateUtils.yyyyMMddHHmmss(folder.minTs.ts), DateUtils.yyyyMMddHHmmss(folder.maxTs.ts), folder.count,
//      DateUtils.currentyyyyMMddHHmmss).mkString("_"))).mkString("/").toLowerCase
}

object WhHdfsLogPathBase extends Enumeration {
...
//  def whHdfsLogPathBase(s: String) = WhHdfsLogPathBase.withName(s.toLowerCase)
}

case class WhHdfsLogPathFolder(minTs: UmsWatermark, maxTs: UmsWatermark, count: Int)

object WhHdfsLogPathUtils extends WhHdfsLogPathUtils


trait WhHdfsLogPathUtils {
  def whHdfsLogPathFolder(fullPath: String): WhHdfsLogPathFolder = {
    ...
    val minTs = if (lastArray(0).startsWith(HdfsUtils.edpTempPrefix)) lastArray(0).drop(HdfsUtils.edpTempPrefix.length) else lastArray(0)
    val maxTs = lastArray(1)
    WhHdfsLogPathFolder(UmsWatermark(minTs), UmsWatermark(maxTs), CommonUtils.s2int(lastArray(2), 0))
  }
  ...
  def getHdfsLogPathListBetween(fullPathList: List[String], fromTsStr: String = null, toTsStr: String = null): List[String] = {
    ...
    val fromTs = DateUtils.dt2dateTime(fromTsStr)
    val toTs = DateUtils.dt2dateTime(toTsStr)
  ...
  def getHdfsLogPathList(rootBaseNsPath: String,
    ...
    val fullRootBaseNsPath = FileUtils.pfRight(rootBaseNsPath)
    assert(HdfsUtils.isPathExist(config, fullRootBaseNsPath), s"The $fullRootBaseNsPath does not exist")
    ...
    val pathFilter = new PathFilter {
      override def accept(path: Path): Boolean = {
        ...
        if (!HdfsUtils.isParquetPathReady(config, p)) return false
  ...
  def getRootBaseNsList(rootBase: String): List[String] = {
    ...
    if (HdfsUtils.isPathExist(conf, rootBase)) getRootBaseNsArray(conf, new Path(rootBase), 1).toList
    else List.empty
  }
  ...
  def getCompactionPathList(rootBaseNsPath: String, exclude0cnt: Boolean = true): List[String] = {
    ...
    val fullRootBaseNsPath = FileUtils.pfRight(rootBaseNsPath) //TODO: will consider support dbPar/tablePar later
    assert(HdfsUtils.isPathExist(config, fullRootBaseNsPath), s"The $fullRootBaseNsPath does not exist")
    ...
    val pathFilter = new PathFilter {
      override def accept(path: Path): Boolean = {
        ...
        if (!HdfsUtils.isParquetPathReady(config, path.toString)) return false
        if (exclude0cnt && whHdfsLogPathFolder(path.toString).count == 0) return false
        true
      }
    }
  ...
  def getCompactionPathListGroup(rootBaseNs: String): List[List[String]] = {
    ...
    if (pathList.isEmpty) return List.empty
    val pathListGroup = pathList.aggregate(mutable.HashMap[String, List[String]]())(seqop, combop)
    (pathListGroup - DateUtils.currentyyyyMMddHHmmss.take(8)).values.toList.filter(_.length > 1)
  }

  def isEdpTempPath(path: String): Boolean = path.split("/").last.startsWith(HdfsUtils.edpTempPrefix)
  ...
  private def isValidWhPath(fullPath: String): Boolean = {
    ...
    val path = tmp.last
    val directPath = if (path.startsWith(HdfsUtils.edpTempPrefix)) path.drop(HdfsUtils.edpTempPrefix.length) else path
    val lastArray = directPath.split("_")
    if (lastArray.length != 4 ||
      !DateUtils.isValidTimeFormat(lastArray(0), DtFormat.TS_NOD_SEC) ||
      !DateUtils.isValidTimeFormat(lastArray(1), DtFormat.TS_NOD_SEC) ||
      !DateUtils.isValidTimeFormat(lastArray(3), DtFormat.TS_NOD_SEC)) return false
    true
  }
