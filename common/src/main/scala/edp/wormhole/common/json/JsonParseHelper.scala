  ...
  def parseData2CorrectType(dataType: UmsFieldType, field: String, name: String): (String, Any) = {
    if (name == UmsSysField.OP.toString || name == UmsSysField.ACTIVE.toString) {
      ...
      dataType match {
        ...
        case UmsFieldType.DATE => if (isNull(field) || field.isEmpty) (name, null) else (name,if(field.trim.indexOf(".")>0 && field.trim.length()-field.indexOf(".")<=3) {
          val len=field.trim.length()-field.trim.indexOf(".")
          (0 until len).foldLeft(field.trim)((soFar, i) => soFar + "0")
        } else  field.trim)
