...
class UmsProcessElement(sourceSchemaMap: Map[String, (TypeInformation[_], Int)], namespaceIdConfig: NamespaceIdConfig, jsonSourceParseMap: Map[(UmsProtocolType, String), (Seq[UmsField], Seq[FieldInfo], ArrayBuffer[(String, String)])], kafkaDataTag: OutputTag[String]) extends ProcessFunction[(String, String, String, Int, Long), Row]{
  ...
  override def processElement(value: (String, String, String, Int, Long), ctx: ProcessFunction[(String, String, String, Int, Long), Row]#Context, out: Collector[Row]): Unit = {
    ...
    try {
      ...
      if (jsonSourceParseMap.contains((protocolType, namespace))) {
        ...
        if (FlinkSchemaUtils.matchNamespace(ums.schema.namespace, namespaceIdConfig.sourceNamespace) && ums.payload.nonEmpty && ums.schema.fields.nonEmpty)
