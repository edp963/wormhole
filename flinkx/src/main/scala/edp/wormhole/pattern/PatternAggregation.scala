...
class PatternAggregation(input: Seq[Iterable[Row]], fieldName: String, schemaMap: Map[String, (TypeInformation[_], Int)]) {
  ...
  private def max() = {
    fieldType match {
      ...
      case Types.SQL_DATE => input.flatten.map(row => dt2sqlDate(row.getField(fieldIndex).asInstanceOf[String])).max(Ordering[Date])
      case Types.SQL_TIMESTAMP => input.flatten.map(row => dt2timestamp(row.getField(fieldIndex).asInstanceOf[String])).max(Ordering[Timestamp])
  ...
  private def min() = {
    fieldType match {
      ...
      case Types.SQL_DATE => input.flatten.map(row => dt2sqlDate(row.getField(fieldIndex).asInstanceOf[String])).min(Ordering[Date])
      case Types.SQL_TIMESTAMP => input.flatten.map(row => dt2timestamp(row.getField(fieldIndex).asInstanceOf[String])).min(Ordering[Timestamp])
