...
class PatternOutput(output: JSONObject, schemaMap: Map[String, (TypeInformation[_], Int)]) extends java.io.Serializable {
  ...
  private def maxRow(input: Seq[Iterable[Row]]) = {
    ...
    fieldType match {
      ...
      case Types.SQL_DATE => input.flatten.maxBy(row => dt2sqlDate(row.getField(fieldIndex).asInstanceOf[String]))(Ordering[Date])
      case Types.SQL_TIMESTAMP => input.flatten.maxBy(row => dt2timestamp(row.getField(fieldIndex).asInstanceOf[String]))(Ordering[Timestamp])
  ...
  private def minRow(input: Seq[Iterable[Row]]) = {
    ...
    fieldType match {
      ...
      case Types.SQL_DATE => input.flatten.minBy(row => dt2sqlDate(row.getField(fieldIndex).asInstanceOf[String]))(Ordering[Date])
      case Types.SQL_TIMESTAMP => input.flatten.minBy(row => dt2timestamp(row.getField(fieldIndex).asInstanceOf[String]))(Ordering[Timestamp])
