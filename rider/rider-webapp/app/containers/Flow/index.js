...
export class Flow extends React.Component {
  ...
  searchOperater() {
    ...
    if (columnNameText !== "") {
      ...
      if (
        columnNameText === "startedTime" ||
        columnNameText === "stoppedTime"
      ) {
        this.onRangeTimeSearch(
          columnNameText,
          startTimeTextState,
          endTimeTextState,
          visibleBool
    ...
    if (selectedRowKeys.length > 0) {
      ...
      this.props.onOperateUserFlow(
        ...
        result => {
          ...
          const languagetextSuccess =
            locale === "en" ? "successfully!" : "成功！";
        ...
        result => {
          message.error(
            `${locale === "en" ? "Operation failed:" : "操作失败："}${result}`,
            3
          );
        }
      );
      ...
      message.warning(
        `${locale === "en" ? "Please select Flow!" : "请选择 Flow！"}`,
        3
      );
    }
  };
      ...
      result => {
        ...
        if (action === "delete") {
          ...
          if (result.msg.includes("failed")) {
            ...
            action === "renew"
              ? message.success(
                  locale === "en" ? "Renew successfully！" : "生效！",
                  3
                )
              : message.success(`${singleMsg}${languagetextSuccess}`, 3);
          }
  ...
  refreshPerformance = (
    performanceMenuRefreshChosen = this.state.performanceMenuRefreshChosen
  ) => {
    const { projectIdGeted } = this.props;
    const { chosenFlowId } = this.state;
  ...
  submitDrift = () => {
    ...
    if (Object.is(id, "")) {
      message.warn(
        `${locale === "en" ? "Please select others:" : "请选择其他"}`,
        3
      );
      return;
    }
  ...
  handleEditStartOk = e => {
    ...
    const { projectIdGeted, locale } = this.props;
    const offsetText =
      locale === "en" ? "Offset cannot be empty" : "Offset 不能为空！";
    this.setState({ unValidate: true }, () => {
      this.flowStartForm.validateFieldsAndScroll((err, values) => {
        if (!err || err.newTopicName) {
          ...
          if (!flowStartFormData) {
            if (!values.udfs) {
              ...
              if (values.udfs.find(i => i === "-1")) {
                // 全选
                const startUdfValsOrigin = startUdfVals.filter(
                  k => k.id !== -1
                );
            ...
            const mergedData = {};
            const autoRegisteredData = this.formatTopicInfo(
              flowStartFormData,
              "auto",
              values,
              offsetText
            );
            const userDefinedData = this.formatTopicInfo(
              userDefinedTopics,
              "user",
              values,
              offsetText
          ...
          actionTypeMsg =
            locale === "en" ? "Start Successfully!" : "启动成功！";
          ...
          this.props.onStartFlinkFlow(
            ...
            result => {
              const failText =
                locale === "en" ? "Operation failed:" : "操作失败：";
              message.error(`${failText} ${result}`, 3);
  ...
  onChangeEditSelect = () => {
    ...
    for (let i = 0; i < flowStartFormData.length; i++) {
      const partitionAndOffset = flowStartFormData[
        i
      ].consumedLatestOffset.split(",");

      for (let j = 0; j < partitionAndOffset.length; j++) {
        this.flowStartForm.setFieldsValue({
          [`${flowStartFormData[i].name}_${j}_auto`]: partitionAndOffset[
            j
          ].substring(partitionAndOffset[j].indexOf(":") + 1),
          [`${flowStartFormData[i].name}_${
            flowStartFormData[i].rate
          }_rate`]: flowStartFormData[i].rate
        });
      }
    }
    for (let i = 0; i < userDefinedTopics.length; i++) {
      const partitionAndOffset = userDefinedTopics[
        i
      ].consumedLatestOffset.split(",");
      for (let j = 0; j < partitionAndOffset.length; j++) {
        this.flowStartForm.setFieldsValue({
          [`${userDefinedTopics[i].name}_${j}_user`]: partitionAndOffset[
            j
          ].substring(partitionAndOffset[j].indexOf(":") + 1),
          [`${userDefinedTopics[i].name}_${
            userDefinedTopics[i].rate
          }_rate`]: userDefinedTopics[i].rate
        });
      }
    }
  };
          ...
          if (
            ...
            if (filteredInfo.status && filteredInfo.streamStatus) {
              if (
                ...
                filteredInfo.streamStatus.length !== 0 &&
                filters.status.length !== 0
              ) {
  ...
  handleEndOpenChange = status =>
    this.setState({ filterDatepickerShown: status });
      ...
      this.setState(
        ...
        () => {
          const requestValue = {
            projectId: record.projectId,
            streamId:
              typeof record.streamId === "object"
                ? record.streamIdOrigin
                : record.streamId,
            flowId: record.id,
          ...
          this.props.onLoadFlowDetail(requestValue, result =>
            this.setState({ showFlowDetails: result })
          );
        }
      );
    }
  ...
  render() {
          ...
          <div className="custom-filter-dropdown">
            <Input
              ...
              onPressEnter={this.onSearch(
                "projectName",
                "searchTextFlowProject",
                "filterDropdownVisibleFlowProject"
              )}
            />
            <Button
              type="primary"
        ...
        render: (text, record) => {
          ...
          return (
            <span
              ...
              onClick={this.filterStreamId(streamId)}
            >
              {record.streamId}
            </span>
          );
          ...
          <div className="custom-filter-dropdown">
            <Input
              ...
              onPressEnter={this.onSearch(
                "streamType",
                "searchTextStreamType",
                "filterDropdownVisibleStreamType"
              )}
            />
            <Button
              type="primary"
      ...
      },
      {
        ...
        render: (text, record) => {
          ...
          if (roleType === "admin") {
            ...
            const sureDeleteFormat = (
              <FormattedMessage {...messages.flowSureDelete} />
            );
            const sureRenewFormat = (
              <FormattedMessage {...messages.flowSureRenew} />
            );
            const driftFormat = (
              <FormattedMessage {...messages.flowTableDrift} />
            );
            const chartFormat = (
              <FormattedMessage {...messages.flowTableChart} />
            );
            const errorListFormat = (
              <FormattedMessage {...messages.flowErrorList} />
            );
              ...
              strLog = (
                <Tooltip title="logs">
                  <Button
                    shape="circle"
                    type="ghost"
                    onClick={this.onShowLogs(record)}
                  >
            ...
            let strChart = (
              <Tooltip title={chartFormat}>
                <Button
                  ...
                  shape="circle"
                  type="ghost"
                  onClick={this.onShowPerformance(record)}
                />
              </Tooltip>
            );
            if (
              record.streamType === "spark" ||
              record.streamTypeOrigin === "spark"
            ) {
            ...
            if (
              (record.streamType === "spark" ||
                record.streamTypeOrigin === "spark") &&
              (record.functionType === "default" ||
                record.functionTypeOrigin === "default")
            ) {
              strDrift = record.disableActions.includes("drift") ? (
                ...
                <Tooltip title={driftFormat}>
                  <Button
                    shape="circle"
                    type="ghost"
                    onClick={this.onShowDrift(record)}
                  >
                    <i className="iconfont icon-sstransfer" />
                  </Button>
                </Tooltip>
            ...
            let strErrorList = (
              <Tooltip title={errorListFormat}>
                <Button
                  ...
                  shape="circle"
                  type="ghost"
                  onClick={this.onShowErrorList(record)}
                />
              </Tooltip>
            );
              ...
              <span>
                ...
                <Tooltip title={copyFormat}>
                  <Button
                    ...
                    shape="circle"
                    type="ghost"
                    onClick={this.onCopyFlow(record)}
                  />
                </Tooltip>
                {strStart}
                {strStop}
          ...
          if (showFlowDetails) {
            const topicTemp =
              showFlowDetails.topicInfo &&
              showFlowDetails.topicInfo.autoRegisteredTopics;
            const topicUserTemp =
              showFlowDetails.topicInfo &&
              showFlowDetails.topicInfo.userDefinedTopics;
            let topicFinal = "";
            let topicUserFinal = "";
            if (topicTemp) {
          ...
          return (
            <span className="ant-table-action-column">
              <Tooltip
                title={<FormattedMessage {...messages.flowViewDetails} />}
              >
                <Popover
                  placement="left"
    ...
    const menuItems = (
      <Menu
        onClick={this.handleMenuClick(selectedRowKeys)}
        className="ri-workbench-select-dropdown"
      >
        ...
        <Menu.Item key="menuStop">
          <i
            className="iconfont icon-8080pxtubiaokuozhan100"
            style={{ fontSize: "12px" }}
          />{" "}
        ...
        <span>
          ...
          <Dropdown trigger={["click"]} overlay={menuItems}>
            <Button type="ghost" className="flow-action-btn">
              <FormattedMessage {...messages.flowBatchAction} />{" "}
              <Icon type="down" />
            </Button>
          </Dropdown>
        </span>
      ...
      <div>
        ...
        >
          ...
          <Dropdown overlay={menuRefresh}>
            <Button style={{ marginLeft: 8 }}>
              {this.state.performanceMenuRefreshChosen.label}{" "}
              <Icon type="down" />
            </Button>
          </Dropdown>
        </div>
        <Line
          ...
          id="throughput"
          options={this.state.throughputChartOpt}
        />
        <Line
          style={{ marginBottom: "5px" }}
          id="records"
          options={this.state.recordsChartOpt}
        />
        <Line
          style={{ height: "250px" }}
          id="latency"
          options={this.state.latencyChartOpt}
        />
      </div>
    ) : (
      ""
    ...
    return (
      <div className={`ri-workbench-table ri-common-block ${className}`}>
        ...
        <h3 className="ri-common-block-title">
          <Icon type="bars" /> Flow{" "}
          <FormattedMessage {...messages.flowTableList} />
        </h3>
...
export function mapDispatchToProps(dispatch) {
  return {
    ...
    onLoadUserAllFlows: (projectId, resolve) =>
      dispatch(loadUserAllFlows(projectId, resolve)),
    onLoadAdminSingleFlow: (projectId, resolve) =>
      dispatch(loadAdminSingleFlow(projectId, resolve)),
    onOperateUserFlow: (values, resolve, reject) =>
      dispatch(operateUserFlow(values, resolve, reject)),

    onEditLogForm: (flow, resolve) => dispatch(editLogForm(flow, resolve)),
    onSaveForm: (flowId, taskType, value, resolve) =>
      dispatch(saveForm(flowId, taskType, value, resolve)),
    onCheckOutForm: (flowId, taskType, startDate, endDate, value, resolve) =>
      dispatch(
        checkOutForm(flowId, taskType, startDate, endDate, value, resolve)
      ),
    ...
    onLoadSourceInput: (flowId, taskType, resolve) =>
      dispatch(loadSourceInput(flowId, taskType, resolve)),
    onLoadFlowDetail: (requestValue, resolve) =>
      dispatch(loadFlowDetail(requestValue, resolve)),
    ...
    onLoadLastestOffset: (
      ...
      dispatch(
        loadLastestOffset(projectId, streamId, resolve, type, topics, tabType)
      ),
    ...
    onStartFlinkFlow: (projectId, id, topicResult, action, resolve, reject) =>
      dispatch(
        startFlinkFlow(projectId, id, topicResult, action, resolve, reject)
      ),
    onStopFlinkFlow: (projectId, id, topicResult, action, resolve, reject) =>
      dispatch(
        stopFlinkFlow(projectId, id, topicResult, action, resolve, reject)
      ),
    onLoadAdminLogsInfo: (projectId, flowId, resolve) =>
      dispatch(loadAdminLogsInfo(projectId, flowId, resolve)),
    onLoadLogsInfo: (projectId, flowId, resolve) =>
      dispatch(loadLogsInfo(projectId, flowId, resolve)),
    onLoadDriftList: (projectId, flowId, resolve) =>
      dispatch(loadDriftList(projectId, flowId, resolve)),
    ...
    onSearchFlowPerformance: (projectId, flowId, startTime, endTime, resolve) =>
      dispatch(
        postFlowPerformance(projectId, flowId, startTime, endTime, resolve)
      ),
    jumpStreamToFlowFilter: streamFilterId =>
      dispatch(jumpStreamToFlowFilter(streamFilterId)),
    onLoadErrorList: (project, flowId, resolve) =>
      dispatch(getErrorList(project, flowId, resolve))
  };
}
