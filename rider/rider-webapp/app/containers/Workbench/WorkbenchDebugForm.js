...
import { postUserTopic } from '../Manager/action'

export class WorkbenchDebugForm extends React.Component {
  constructor (props) {
    super(props)
    this.state = {
      data: [] || '',
  ...
  render () {
    const { form, startUdfValsOption, currentUdfVal, locale } = this.props
                ...
                <div className="rate-topic-info-wrapper">
                  <div className="rate-class">
                    <Col span={24} className="card-content required-offset">
                      Rate (<FormattedMessage {...messages.workbenchModalRate} />)
                    </Col>
                  </div>
          ...
          : data.map(i => {
            ...
            if (i.consumedLatestOffset) {
              ...
              parOffInput = partitionOffsetsArr.map((g, index) => {
                ...
                const applyFormat = <FormattedMessage {...messages.workbenchModalApply} />
                return (
            ...
            const applyAllText = <FormattedMessage {...messages.workbenchModalApplyAll} />
            const cardContent = (
    ...
    const udfChildren = startUdfValsOption.map(i => (<Select.Option key={i.id} value={`${i.id}`}>{i.functionName}</Select.Option>))
        ...
        <Row>
          <Card title="UDFs：" className={'stream-start-form-udf-style'}>
            <div className="udf-info-wrapper">
              <div className={`hide selected-udf-class`}>Selected UDFs：{currentUdfsShow}</div>
...
export default Form.create({wrappedComponentRef: true})(connect(mapStateToProps, mapDispatchToProps)(WorkbenchDebugForm))
