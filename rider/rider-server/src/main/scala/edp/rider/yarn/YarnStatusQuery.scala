...
object YarnStatusQuery extends RiderLogger {
  ...
  def getAllYarnAppStatus(fromTime: String, appNames: Seq[String]): Map[String, AppResult] = {
    ...
    if (rmUrl != "") {
      val url = s"http://${rmUrl.stripPrefix("http://").stripSuffix("/")}/ws/v1/cluster/apps?states=accepted,running,killed,failed,finished&queue=$queueName&startedTimeBegin=$fromTimeLong&applicationTypes=spark,apache%20flink"
