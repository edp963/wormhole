...
object SubmitSparkJob extends App with RiderLogger {
  ...
  def getConfig(streamId: Long, streamName: String, brokers: String, launchConfig: LaunchConfig) = {
    ...
    val kafkaOutput = new String(baseDecoder.decodeBuffer("ImthZmthX291dHB1dCI6eyJmZWVkYmFja190b3BpY19uYW1lIjoi")) + RiderConfig.consumer.feedbackTopic + new String(baseDecoder.decodeBuffer("IiwiYnJva2VycyI6Ig==")) + RiderConfig.consumer.brokers + new String(baseDecoder.decodeBuffer("In0s"))
