...
object SparkStatusQuery extends RiderLogger {
  ...
  def getAllYarnAppStatus(fromTime: String): List[AppResult] = {
    ...
    if (rmUrl != "") {
      val url = s"http://${rmUrl.stripPrefix("http://").stripSuffix("/")}/ws/v1/cluster/apps?states=accepted,running,killed,failed,finished&&startedTimeBegin=$fromTimeLong&&applicationTags=${RiderConfig.spark.appTags}&&applicationTypes=spark"
