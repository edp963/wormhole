...
object RiderConfig {
  ...
  lazy val wormholeJarPath = getStringConfig("spark.wormhole.jar.path", s"${RiderConfig.riderRootPath}/lib/wormhole-ums_1.3-sparkx_2.2.0-0.3.5-SNAPSHOTS-jar-with-dependencies.jar")
  lazy val wormholeKafka08JarPath = getStringConfig("spark.wormhole.kafka08.jar.path", s"${RiderConfig.riderRootPath}/lib/wormhole-ums_1.3-sparkx_2.2.0-0.3.5-SNAPSHOTS-jar-with-dependencies-kafka08.jar")
  ...
  def getDurationConfig(path: String, default: Duration): Duration = {
    if (config.hasPath(path) && !config.getIsNull(path))
      config.getDuration(path, TimeUnit.SECONDS).millis
    else default
  }

  def getFiniteDurationConfig(path: String, default: FiniteDuration): FiniteDuration = {
    if (config.hasPath(path) && !config.getIsNull(path))
      config.getDuration(path, TimeUnit.MILLISECONDS).seconds
    else default
  }
