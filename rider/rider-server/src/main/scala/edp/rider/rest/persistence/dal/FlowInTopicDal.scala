...
//  def getFlowAutoRegisteredTopics(flowId: Long): Seq[FlowTopicAllOffsets] = {
//    Await.result(db.run((flowInTopicTable.filter(_.flowId === flowId) join nsDatabaseTable on (_.nsDatabaseId === _.id))
//      .map {
