...
@Path("/app/namespaces")
class NamespaceAppRoutes(modules: ConfigurationModule with PersistenceModule with BusinessModule with RoutesModuleImpl) extends Directives {

  lazy val routes: Route = postNamespaceRoute ~ getSinkNsByUser
  ...
  def getSinkNsByUser: Route = modules.namespaceAdminService.getSinkNsByUser(basePath)
