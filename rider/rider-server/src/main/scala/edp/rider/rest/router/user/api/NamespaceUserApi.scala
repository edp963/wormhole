                  ...
                  if (session.projectIdList.contains(id)) {
                    (sourceType, sinkType, transType) match {
                      case (Some(source), None, None) =>
                        onComplete(relProjectNsDal.getJobSourceNamespaceByProjectId(id, source).mapTo[Seq[Namespace]]) {
                          case Success(nsSeq) =>
                      ...
                      case (None, Some(sink), None) =>
                        onComplete(relProjectNsDal.getSinkNamespaceByProjectId(id, sink).mapTo[Seq[Namespace]]) {
                          case Success(nsSeq) =>
              ...
              session =>
                if (session.roleType != "user") {
                  ...
                  if (session.projectIdList.contains(projectId)) {
                    (sourceType, sinkType, transType) match {
                      case (Some(source), None, None) =>
                        onComplete(relProjectNsDal.getFlowSourceNamespaceByProjectId(projectId, streamId, source).mapTo[Seq[Namespace]]) {
                          case Success(nsSeq) =>
