                    ...
                    prepare(None, Some(appFlow), session, projectId, Some(streamId)) match {
                      case Right(tuple) =>
                        ...
                        try {
                          ...
                          if (startFlow(stream.id, stream.streamType, flow.id, flow.sourceNs, flow.sinkNs, flow.consumedProtocol, flow.sinkConfig.getOrElse(""), flow.tranConfig.getOrElse(""), flow.tableKeys.getOrElse(""), flow.updateBy)) {
