          ...
          session =>
            if (session.roleType != "user") {
              ...
              if (session.projectIdList.contains(projectId)) {
                ...
                val projectName = jobDal.adminGetRow(job.projectId)
                complete(OK, ResponseJson[FullJobInfo](getHeader(200, session), FullJobInfo(job, projectName)))
              } else {
                riderLogger.error(s"user ${session.userId} doesn't have permission to access the project $projectId.")
                complete(OK, getHeader(403, session))
              }
...
def stopRoute(route: String): Route = path(route / LongNumber / "jobs" / LongNumber /  "stop") {
  (projectId, jobId) =>
    put {
      authenticateOAuth2Async[SessionClass]("rider", AuthorizationProvider.authorize) {
        session =>
          if (session.roleType != "user") {
            ...
            try {
              ...
              if (job.isEmpty) {
              ...
              } else if (job.get.status == "starting") {
                ...
                complete(OK, ResponseJson[FullJobInfo](getHeader(200, null), FullJobInfo(updateJob, projectName)))
              }
            } catch {
              case ex: Exception =>
