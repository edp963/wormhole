...
class CacheEffectDal(cacheHitTableQ: TableQuery[CacheEffectTable]) extends BaseDalImpl[CacheEffectTable, CacheEffectEntity](cacheHitTableQ)
  with ConfigModuleImpl with DBDriverModuleImpl{

  private val logger = Logger.getLogger(this.getClass)

  def updateOrInsert(rows: Seq[CacheEffectEntity]) = {
    rows.foreach { row =>
      ...
      try {
        if( Await.result(super.findByFilter(_.streamId === row.streamId), FiniteDuration(180, SECONDS) ).isEmpty){
          db.run(cacheHitTableQ += row).mapTo[Seq[CacheEffectEntity]]
          logger.info(s"insert row \n")
        } else {
          db.run(cacheHitTableQ.filter(_.streamId === row.streamId).update(row)).mapTo[Seq[CacheEffectEntity]]
