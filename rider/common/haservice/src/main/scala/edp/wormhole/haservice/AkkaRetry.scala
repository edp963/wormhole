...
class AkkaRetry(val retryTimes: Int, retryTimeOut: FiniteDuration, retryInterval: FiniteDuration, actorHandler: ActorRef) extends Actor {
      ...
      (actorHandler ? message) (retryTimeOut) onComplete {
        ...
        case Failure(err) =>
          if (triesLeft == 1) {
            ...
            logger.error(s" akka retry failed ${err.getMessage}")
          }
      }
      if (triesLeft > 1)
        context.system.scheduler.scheduleOnce(retryInterval, self, RetryEntry(originalSender, message, triesLeft - 1))

    case msg@_ => logger.error(s"No handling defined for message: $msg")
  }
