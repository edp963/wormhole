...
//  }
  ...
  def spark2umsSchema(sparkType: StructType, namespace: String = empty): UmsSchema =
  ...
  def spark2umsValue(umsFieldType: UmsFieldType, value: Any): Any = if (value == null) null
    ...
    case UmsFieldType.DECIMAL => value.asInstanceOf[JavaBigDecimal].stripTrailingZeros()
    case UmsFieldType.BINARY => value.asInstanceOf[JavaByteArray]
