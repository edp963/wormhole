...
object RowkeyTool {
  ...
  def generateKeyFieldsSchema(originalData: mutable.Seq[Row], patternContentList: mutable.Seq[RowkeyPatternContent]): mutable.Seq[(Boolean, Int, String)] = {
  ...
  def generateRowKeyDatas(keyFieldsSchema:Seq[(Boolean, Int, String)],row:Row): Seq[String] ={
    ...
    keyFieldsSchema.map(fieldDesc => {
      if (fieldDesc._1) {
        val value = WormholeUtils.getFieldContentByType(row, schema, fieldDesc._2).toString
