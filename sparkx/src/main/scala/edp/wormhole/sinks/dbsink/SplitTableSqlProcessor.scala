...
class SplitTableSqlProcessor(sinkProcessConfig: SinkProcessConfig, schemaMap: collection.Map[String, (Int, UmsFieldType, Boolean)], specificConfig: DbConfig, sinkNamespace: String, connectionConfig: ConnectionConfig) extends EdpLogging {
  ...
  def checkDbAndGetInsertUpdateDeleteList(keysTupleMap: mutable.HashMap[String, Seq[String]]): (Seq[Seq[String]], Seq[Seq[String]], Seq[Seq[String]], Seq[Seq[String]], Seq[Seq[String]], Seq[Seq[String]], Seq[Seq[String]], Seq[Seq[String]]) = {
    ...
    try {
      ...
      logInfo("@masterSql: " + masterSql)
      logInfo("@subSql: " + subSql)
  ...
  def contactDb(tupleList: Seq[Seq[String]], opType: String): Seq[Seq[String]] = {
    ...
    if (errorTuple2List.nonEmpty) errorTuple2List.foreach(data => logError("opType:" + opType + ",data:" + data))
    errorTuple2List
  }
  ...
  def specialExecuteSql(tupleList: Seq[Seq[String]], masterSql: String, subSql: String, batchSize: Int): List[Seq[String]] = {
    def setPlaceholder(tuple: Seq[String], ps: PreparedStatement, sqlType: String, sql: String, insertFieldNames: List[String], updateFieldNames: List[String]) = {
      ...
      if (sqlType == DataTable) {
        ...
        if (sql.startsWith("I")) {
          ...
          val opValue: Int = if (UmsOpType.umsOpType(fieldValue(OP.toString, schemaMap, tuple).toString) == UmsOpType.DELETE) UmsActiveType.INACTIVE
    ...
    try {
      ...
      logInfo(s"@write list.size:${tupleList.length} masterSql $masterSql")
      logInfo(s"@write list.size:${tupleList.length} subSql $subSql")
      ...
      if (psMaster != null)
        try {
          ...
          case e: Throwable => logError("psMaster.close", e)
        }
      if (psSub != null)
        try {
          psSub.close()
        } catch {
          case e: Throwable => logError("psSub.close", e)
        }
      if (null != conn)
        try {
          conn.close()
